...
3. `docker compose build postgres proxy` then `docker compose up -d postgres`. The initializer creates separate maia and n8n databases and runtime roles. PostgreSQL has no host port. The owner/migration role is reserved for private maintenance.
